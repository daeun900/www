...
$(document).ready(function () {
    var article = $('.content_area .article');  //모든 질문 답변 리스트
      ...
      $('.all').toggle(function(e){
            ...
            article.parents().parents().find('.all').html('<span>모두닫기<i class="fa-solid fa-circle-minus"></i></span>');
      },function(e){
            e.preventDefault(); 
            article.find('.a').slideUp(100);
            article.removeClass('show').addClass('hide');
            article.find('span.plus').html('<i class="fas fa-chevron-down"></i>');
            article.parents().parents().find('.all').html('<span>모두열기<i class="fa-solid fa-circle-plus"></i></span>');
      });  
    
    //tap menu
    
        var cnt=$('.content_area li').size();
